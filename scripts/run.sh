...
run_local() {
    ...
    cd "$SCRIPT_DIR"
    APP_ENV=demo ENABLE_TOTP_TEST_ENDPOINT=true ENABLE_QUERY_CONSOLE=true "$PYTHON_CMD" -m database_server > /tmp/db_server.log 2>&1 &
    DB_PID=$!
    ...
    cd "$SCRIPT_DIR"
    APP_ENV=demo SSL_VERIFY=false "$PYTHON_CMD" -m proxy_clone > /tmp/proxy_server.log 2>&1 &
    PROXY_PID=$!
